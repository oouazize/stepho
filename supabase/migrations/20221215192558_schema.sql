...
create policy "Assets can be read by users of the organizations to which it belongs" on assets
  for all
    using (exists (
      select
        1
      from
        memberships
      where
        user_id = auth.uid () and assets.organization_id = memberships.organization_id));
