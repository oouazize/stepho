...
alter table _assets enable row level security;

select
  tests.rls_enabled('assets');

create policy "Can insert assets with an active subscription" on _assets
...
create policy "Can read assets if they belong to the organization" on _assets
...
select
  lives_ok($$ insert into _assets(
      name, organization_id)
    values (
      'Asset 1', makerkit.get_organization_id(
        'Organization'));

$$,
'can insert assets with an active subscription');

select
  throws_ok($$ insert into _assets(
      name, organization_id)
    values (
      'Asset 2', makerkit.get_organization_id(
        'Organization 2')) $$, 'new row violates row-level security policy for table "_assets"');
...
$$,
'new row violates row-level security policy for table "_assets"');
